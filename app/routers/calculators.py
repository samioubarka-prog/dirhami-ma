...
async def calculate_investment(data: InvestmentCalc):
    ...
    if pmt > 0:
        future_value = data.initial_amount * (1 + r) ** n + pmt * ((1 + r) ** n - 1) / r
    else:
...
async def calculate_loan(data: LoanCalc):
    ...
    if data.type == "amortissable":
        if monthly_rate > 0:
            monthly_payment = data.amount * (monthly_rate * (1 + monthly_rate) ** n_months) / ((1 + monthly_rate) ** n_months - 1)
        else:
...
async def calculate_retirement(data: RetirementCalc):
    ...
    future_value = data.current_savings * (1 + r) ** months + data.monthly_savings * ((1 + r) ** months - 1) / r
